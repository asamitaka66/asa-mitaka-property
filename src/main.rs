...
/// ====== SESSION CLAIMS (optional session token issued on verify) ======
#[derive(Debug, Serialize, Deserialize)]
struct SessionClaims {
    lic_id: String,
    exp: i64,
...
struct CreateLicenseRequest {
    /// Duration in seconds: 1..=315360000 (10 years)
    duration_seconds: i64,
    /// Optional note for tracking (not stored in current model; kept for forward-compat)
    note: Option<String>,
}
...
/// ====== OPTIONAL BLACKLIST (for session tokens, if you later revoke sessions) ======
struct TokenBlacklist {
    tokens: HashSet<String>,
}
impl TokenBlacklist {
...
fn clamp_duration_seconds(s: i64) -> i64 {
    let min = 1_i64;
    let max = 315_360_000_i64; // 10 years
    s.clamp(min, max)
}
...
fn load_server_key() -> ServerKey {
    ...
    // Set JWT_SECRET to a strong random string (32+ bytes).
    let secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
        // fallback (fine for dev, not for production)
        "dev_secret_change_me_dev_secret_change_me_32b".to_string()
    });
...
impl Api {
    ...
    async fn list_licenses(
        ...
        let total_row: Option<CountRow> = db
            .query("SELECT count() AS total FROM license GROUP ALL;")
            .await
            .map_err(|_| poem::Error::from_status(StatusCode::INTERNAL_SERVER_ERROR))?
            .take(0)
        ...
        let total = total_row.map(|r| r.total).unwrap_or(0);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // DB (Railway-safe default path)
    let db_path = env_string("DB_PATH", "/tmp/db");
    let db = Surreal::new::<RocksDb>(db_path).await?;
    db.use_ns("test").use_db("test").await?;

    // Swagger server URL (optional)
    let public_url =
        std::env::var("PUBLIC_URL").unwrap_or_else(|_| "http://localhost:3000".to_string());
    let api_service = OpenApiService::new(Api, "Jammy Backend", "1.0")
        .server(format!("{}/api", public_url));
    let ui = api_service.swagger_ui();

    // Static dashboard folder
    // Put index.html/styles.css/app.js in this folder.
    let static_dir = env_string("STATIC_DIR", ".");
    let static_files = StaticFiles::new(static_dir).index_file("index.html");
